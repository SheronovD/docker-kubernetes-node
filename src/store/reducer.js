	...
	switch(type) {
		...
		case MOVE_DISH_BACK:
			...
			return {
				...
				cooks: state.cooks.map((cook, index) => {
					if (index === payload.cookId - 1) {  // cookId - 1 - повар, куда перемещаем блюдо
						return {
							...cook,
					...
					if (index === payload.cookId) {
						return {
							...cook,
							dishes: backDishes  // backDishes - это все блюда, кроме того, которое мы перемещаем
						};
					}
		...
		case MOVE_DISH_FORWARD:
			...
			return {
				...state,
				cooks: state.cooks.map((cook, index) => {
					if (index === payload.cookId + 1) {  // cookId + 1 - повар, которому передаем блюдо
						return {
							...cook,
